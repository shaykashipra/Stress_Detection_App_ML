...
//
//            batteryStatusTextView.setText("Battery Level: " + batteryLevel + "%, " + batteryStatus);
//            dndStatusTextView.setText("Do Not Disturb: " + dndStatus);
//        }
//    };
//
//    @Override
...
public class BatteryActivity extends AppCompatActivity {
    ...
    // BroadcastReceiver to handle updates from BatteryService
    private BroadcastReceiver batteryDataReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            int batteryLevel = intent.getIntExtra("batteryLevel", -1);
            ...
            batteryStatusTextView.setText("Battery Level: " + batteryLevel + "%, " + batteryStatus);
            dndStatusTextView.setText("Do Not Disturb Status: " + dndStatus);
        }
    };
    ...
    protected void onResume() {
        super.onResume();
        // Register the receiver with RECEIVER_NOT_EXPORTED to restrict it to this app
        IntentFilter filter = new IntentFilter(BatteryService.ACTION_BATTERY_UPDATE);
        registerReceiver(batteryDataReceiver, filter, Context.RECEIVER_NOT_EXPORTED);
    }
    ...
    protected void onPause() {
        super.onPause();
        // Unregister the receiver to stop listening for updates when the activity is not in the foreground
        unregisterReceiver(batteryDataReceiver);
    }
}
